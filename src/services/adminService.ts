...
export const verifyAdminLoginCode = async (data: { email: string; code: string }) => {
    const response = await api.post('/admin/verify-login-code', data);
    return response.data;
};

export const resendAdminLoginCode = async (data: { email: string }) => {
    const response = await api.post('/admin/resend-login-code', data);
    return response.data;
};
...
export const updateUserVerification = async (userId: string, isVerified: boolean, role: string = 'host') => {
    const response = await api.patch(`/admin/users/${userId}/verification`, { isVerified, role });
    return response.data.data;
};
...
export const updatePlatformFees = async (data: any) => {
