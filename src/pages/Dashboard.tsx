...
import { getDashboardStats, getAllReports, getAllReviews } from '../services/adminService';
...
const Dashboard: React.FC = () => {
    ...
    const { data: reviewsData, isLoading: isLoadingReviews } = useQuery({
        queryKey: ['adminReviewsDashboard'],
        queryFn: () => getAllReviews({ limit: 5 })
    });
    ...
    const stats = [
        ...
        {
            label: 'Avis & Évaluations',
            value: statsData?.overview?.totalReviews !== undefined
                ? statsData.overview.totalReviews
                : (reviewsData?.total || 0),
            icon: <Star size={24} color="#F59E0B" fill="#F59E0B" fillOpacity={0.2} />,
            change: reviewsData?.reviews?.length ? `${reviewsData.reviews.length} récents` : 'Tous les avis',
            isPositive: true
        },
        {
    ...
    return (
        <div className="dashboard-container">
            ...
            <div className="dashboard-content-grid">
                ...
                {/* Right Column: Notifications des avis & Derniers signalements */}
                <div className="dashboard-side-column" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                    {/* Notifications des avis */}
                    <div className="card dashboard-reviews-card">
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                ...
                                <div>
                                    <h2 className="dashboard-section-title" style={{ margin: 0, fontSize: '16px' }}>
                                        Notifications des avis
                                    </h2>
                                    <span style={{ fontSize: '12px', color: 'var(--light, #717171)' }}>
                                        Derniers avis et notes reçus
                                    </span>
                                </div>
                            </div>
                            <button
                        ...
                        <div className="dashboard-reviews-list">
                            {isLoadingReviews ? (
                                <p className="dashboard-loading-text">Chargement des avis...</p>
                            ) : reviewsData?.reviews?.length > 0 ? (
                                reviewsData.reviews.slice(0, 5).map((review: any) => {
                                    const reviewerName = review.reviewer
                                        ? `${review.reviewer.firstName || ''} ${review.reviewer.lastName || ''}`.trim()
                                        : 'Utilisateur inconnu';
                                    const avatarUrl = review.reviewer?.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(reviewerName)}&background=f0f2f5&color=333`;
                                    const isHost = review.reviewerRole === 'host';
                                            ...
                                            <div className="dashboard-review-top">
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', minWidth: 0 }}>
                                                    <img
                                                        src={avatarUrl}
                                                        alt={reviewerName}
                                                        className="dashboard-review-avatar"
                                                        onError={(e) => {
                                                            (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${encodeURIComponent(reviewerName)}&background=f0f2f5&color=333`;
                                                        }}
                                                    />
                                                    <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
                                                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                                            <span className="dashboard-review-author-name">{reviewerName}</span>
                                                            <span
                                                                style={{
                                                                    fontSize: '10px',
                                                                    fontWeight: 600,
                                                                    padding: '2px 6px',
                                                                    borderRadius: '4px',
                                                                    background: isHost ? '#EDE9FE' : '#E0F2FE',
                                                                    color: isHost ? '#6D28D9' : '#0369A1',
                                                        ...
                                                        <span className="dashboard-review-listing-title">
                                                            {review.listing?.title || 'Logement'}
                                                        </span>
                                                    </div>
                                                </div>
                                                ...
                                                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '2px', flexShrink: 0 }}>
                                                    ...
                                                    <span style={{ fontSize: '11px', color: 'var(--light, #717171)' }}>
                                                        {new Date(review.createdAt).toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit' })}
                                                    </span>
                                                </div>
                                            </div>

                                            <p className="dashboard-review-snippet">
                                                "{review.comment}"
                                            </p>

                                            <div className="dashboard-review-bottom">
                                                <span style={{
                                                    fontSize: '11px',
                                                    color: review.isPublic ? '#15803D' : '#D97706',
                                                    fontWeight: 500
                                                }}>
                                                    {review.isPublic ? '● Avis publié' : '○ En attente'}
                                                </span>
                                                <span style={{ fontSize: '12px', color: 'var(--primary, #FF385C)', display: 'flex', alignItems: 'center', gap: '2px', fontWeight: 500 }}>
                                                    Modérer <ChevronRight size={12} />
                                                </span>
                                            </div>
                                        </div>
                                    );
                                ...
                                <p className="dashboard-empty-text">Aucun avis reçu pour le moment.</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="card dashboard-reports-card">
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                            <h2 className="dashboard-section-title" style={{ margin: 0, fontSize: '16px' }}>Derniers signalements</h2>
                            <button
                                className="btn btn-secondary"
